...
int char_make_new_char_sql(struct char_session_data* sd, char* name_, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style) {
	...
	for (k = 0; k <= MAX_STARTITEM && start_items[k].nameid != 0; k ++) {
		if( SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s` (`char_id`,`nameid`, `amount`, `equip`, `identify`) VALUES ('%d', '%hu', '%d', '%d', '%d')", charserv_table(inventory_table), char_id, start_items[k].nameid, start_items[k].amount, start_items[k].pos, 1) )
			Sql_ShowDebug(sql_handle);
	}
